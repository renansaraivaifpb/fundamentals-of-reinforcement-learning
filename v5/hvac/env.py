...
class ClassroomACEnv(gym.Env):
    ...
    def __init__(
        ...
        cfg = self.config
        # A propagação canônica vive em `ClassroomConfig.__post_init__` — a
        # config é a fonte de verdade, e depender do env para completá-la era o
        # que deixava `physics.heating_enabled` False para todo consumidor que
        # não instanciasse um env (ver a nota de regressão lá).
        # Este bloco permanece como rede idempotente para o caso de a flag ser
        # mutada DEPOIS da construção (`cfg.heating_enabled = True`), padrão
        # usado em alguns testes, que não dispara `__post_init__`.
    ...
    def _get_info(self) -> Dict:
        ...
        return {
            ...
            "demand_kw": (float(np.mean(self.demand.buffer))
                          if self.demand and self.demand.buffer else 0.0),
            "demand_peak_kw": self.demand.peak_kw if self.demand else 0.0,
        }
    ...
    def step(self, action) -> Tuple[np.ndarray, float, bool, bool, Dict]:
        ...
        if self.demand is not None:
            self.demand.update(cfg.physics.electrical_kw_signed(self.load))
    ...
    def _comfort_reward(self, temp: float) -> float:
        ...
        if cfg.comfort_type == "band":
            ...
            # O gradiente que guia a política dentro da faixa vem do CUSTO
            # (energia, tarifa, folga de demanda), não do conforto.
            d = abs(temp - cfg.ideal_temp)
            tol = cfg.lab_tolerance
            if d <= tol:
                return cfg.comfort_bonus
            # Parede linear com inclinação alta: sair da faixa é caro e o
            # gradiente nunca zera, então sempre há caminho de volta.
            return cfg.comfort_bonus - cfg.band_wall_slope * (d - tol)
    ...
    def _reward(self, changed: bool, dwell_before_change: int) -> float:
        ...
        # --- Demanda contratada: penaliza exceder a média de 15 min ---
        # Quadrática no excedente para que ultrapassar muito seja
        # desproporcionalmente pior que ultrapassar pouco — reflete o fator
        # punitivo da cobrança de ultrapassagem.
        demanda = 0.0
        if self.demand is not None:
            excesso = max(0.0, float(np.mean(self.demand.buffer))
                          - cfg.demand_contracted_kw) if self.demand.buffer else 0.0
            demanda = -cfg.demand_penalty * excesso ** 2
        ...
        return comfort + energy + change + cold + cycle + rate + demanda
